...
subject to con2{(i,j) in fixed_arcs diff parallel_arcs}: 
   ...
   #h[i] - h[j]  = q[i,j] *((((q[i,j])^2 + eps[i,j]^2)^0.426)) *omega * L[i,j] / ( (exroughness[i,j]^1.852) * (exdiam[i,j])^4.87) ;
...
subject to con4{(i,j) in parallel_arcs}: 
   ...
   #h[i] - h[j]  = q1[i,j] *((((q1[i,j])^2 + eps[i,j]^2)^0.426)) *omega * L[i,j] / ( (exroughness[i,j]^1.852) * (exdiam[i,j])^4.87) ;

subject to con5{(i,j) in parallel_arcs}: 
   h[i] - h[j]  = (q2[i,j])^3 *((((q2[i,j])^2 + eps[i,j]^2)^0.426) /((q2[i,j])^2 + 0.426*eps[i,j]^2)) * sum{k in pipes}(omega * l[i,j,k]/(R[k]^1.852 * d[k]^4.87)) ;
   #h[i] - h[j]  = q2[i,j] *((((q2[i,j])^2 + eps[i,j]^2)^0.426)) * sum{k in pipes}(omega * l[i,j,k]/(R[k]^1.852 * d[k]^4.87)) ;
